function tMod = HTmodSkin(geometry, material, distT, sigmaR, distL)
...
for i = 1:length(distT)
    N = HTrealN(geometry, distT(i));
    if N < 0
        N = 0;
    end
%     q = abs(HTSpar_z(geometry, material, distT(i)));
    load('MoKapIsANaughtyBoy.mat')
    q = polyval(p,distT(i));
